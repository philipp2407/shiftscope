...
def create_tensors_no_labels(path, filtered_indices, channels, path_to_save_tensors_to, size_of_each_tensor,shape=(48,48)):
...
def create_tensor(path, channels, shape, cellNumber, start_idx, end_idx):
    with Container(path, 'r') as seg:  
...
def create_train80_test20_tensors(path_to_seg_file, channels, training_tensor_path, testset_tensor_path, shape=(48,48)):
    ...
    with Container(path_to_seg_file, 'r') as seg:  
        ...
        for i in tqdm(range(16), desc=f"{Colors.GREEN}Creating tensors{Colors.RESET}"):
            ...
            tensorTrain, labelsTrain = create_tensor(path_to_seg_file, channels, shape, num_cells_per_training_tensor, start_idx_train, end_idx_train)
            ...
            tensorVal, labelsVal = create_tensor(path_to_seg_file, channels, shape, num_cells_per_test_tensor, start_idx_val, end_idx_val)
...
def create_train80_val20_tensors_forDDM(path_to_seg_file, labels, channels, training_tensor_path, val_tensor_path, shape=(48,48)):
    ...
    with Container(path_to_seg_file, 'r') as seg:  
        ...
        for i in tqdm(range(16), desc=f"{Colors.GREEN}Creating tensors{Colors.RESET}"):
            ...
            start_idx_train = int(i * num_cells_per_training_tensor) // 4
            end_idx_train = int((i + 1) * num_cells_per_training_tensor) // 4
            tensorTrain, labelsTrain = create_tensor_forDDM(path_to_seg_file, channels, shape, num_cells_per_training_tensor, start_idx_train, end_idx_train, labels)
            np.save(os.path.join(training_tensor_path, f'tensor_{i}.npy'), tensorTrain) #  saves the tensor to disk as a .npy file
            np.save(os.path.join(training_tensor_path, f'labels_{i}.npy'), labelsTrain) # saves the corresponding labels to disk as a .npy file
            del tensorTrain 
            del labelsTrain 
            ...
            tensorVal, labelsVal = create_tensor_forDDM(path_to_seg_file, channels, shape, num_cells_per_validation_tensor, start_idx_val, end_idx_val, labels)
...
# function to create a tensor using a start and end id
def create_tensor_forDDM(path, channels, shape, cellNumber, start_idx, end_idx, labels):
...
def create_train80_test20_tensors_DEBRIS(path_to_seg_file, channels, training_tensor_path, testset_tensor_path, shape=(48,48)):
    ...
    with Container(path_to_seg_file, 'r') as seg:  
        ...
        for i in tqdm(range(16), desc=f"{Colors.GREEN}Creating tensors{Colors.RESET}"):
            ...
            tensorTrain, labelsTrain = create_tensor_DEBRIS(path_to_seg_file, channels, shape, num_cells_per_training_tensor, start_idx_train, end_idx_train)
            np.save(os.path.join(training_tensor_path, f'tensor_{i}.npy'), tensorTrain) #  saves the tensor to disk as a .npy file
            np.save(os.path.join(training_tensor_path, f'labels_{i}.npy'), labelsTrain) # saves the corresponding labels to disk as a .npy file
            del tensorTrain 
            del labelsTrain 
            ...
            tensorVal, labelsVal = create_tensor_DEBRIS(path_to_seg_file, channels, shape, num_cells_per_test_tensor, start_idx_val, end_idx_val)
            np.save(os.path.join(testset_tensor_path, f'tensor_{i}.npy'), tensorVal)
            np.save(os.path.join(testset_tensor_path, f'labels_{i}.npy'), labelsVal)
            del tensorVal 
            del labelsVal 
...
def create_tensor_DEBRIS(path, channels, shape, cellNumber, start_idx, end_idx):
    with Container(path, 'r') as seg:  
        categories = list(seg.content.label.celltype.names)
        # Create one-hot encoding for the labels
        ohc = OneHotEncoder(categories=[categories])
        ...
        neu_idx = np.where(celltype == 'neu')[0]
        debris_idx = np.where(celltype == 'debris')[0]
        # Calculate the number of cells per class to include in the tensors
        num_per_class = cellNumber // 5
        # Select a subset of cells from each class
        ...
        for c in channel:
            c = np.tile(c[..., np.newaxis], 1)
            print(f"Shape before resizing: {c.shape}")  # Before resizing
            c = tf.image.resize(c, size=shape)
            print(f"Shape after resizing: {c.shape}")  # After resizing
            c = tf.squeeze(c)
            tensor_list.append(c)
        # Map cell type labels to integer values
        ...
        # Stack the channels and labels to create the final tensor and label arrays
        # added
        for i, c in enumerate(tensor_list):
            print(f"Channel {i} shape after resizing and before stacking: {c.shape}")
...
# this function creates train test and val tensors from one .seg file (with 80/10/10 split)
# no filtering is done in this class!
def create_train80_val10_test10_tensors(path_to_seg_file, channels, training_tensor_path, val_tensor_path, test_tensor_path, shape=(48,48)):
    #print(f"{Colors.RED}First deleting all files in:\n {Colors.RESET}{training_tensor_path}\n {val_tensor_path}\n {test_tensor_path}.\n {Colors.RED}Type yes to proceed or abort!{Colors.RESET}")
    #userInput = input()
    #if userInput != "yes":
    #    print("Aborting...")
    #    return
    delete_files_in_folder(training_tensor_path)
    delete_files_in_folder(val_tensor_path)
    delete_files_in_folder(test_tensor_path)
    # first need to find out how many cells of each celltype we have!
    with Container(path_to_seg_file, 'r') as seg:  
        categories = list(seg.content.label.celltype.names)
        # Create one-hot encoding for the labels
        ...
        print(f"We can maximally use {numberOfCellsWeCanUse} cells of this dataset if we want it to be equally balanced")
        numberOfCellsPerTensorWeCanUse = numberOfCellsWeCanUse / 16
        num_cells_per_validation_tensor = round(numberOfCellsPerTensorWeCanUse * 0.1)
        num_cells_per_test_tensor = round(numberOfCellsPerTensorWeCanUse * 0.1)
        num_cells_per_training_tensor = numberOfCellsPerTensorWeCanUse - num_cells_per_validation_tensor - num_cells_per_test_tensor
        print(f"{Colors.BLUE}We use a 80/10/10 Train/Val/Test split: Using {num_cells_per_training_tensor * 16} for training, {num_cells_per_validation_tensor * 16} for validation, {num_cells_per_test_tensor*16} for testing{Colors.RESET}")
        for i in tqdm(range(16), desc=f"{Colors.GREEN}Creating tensors{Colors.RESET}"):
            ...
            start_idx_val = int(16 * num_cells_per_training_tensor) // 4 + int(i * num_cells_per_validation_tensor) // 4
            end_idx_val = int(16 * num_cells_per_training_tensor) // 4 + int((i + 1) * num_cells_per_validation_tensor) // 4
            #print(f"{Colors.RED}start idx val is: {start_idx_val}{Colors.RESET}")
            #print(f"{Colors.RED}end idx val is: {end_idx_val}{Colors.RESET}")
            tensorVal, labelsVal = create_tensor(path_to_seg_file, channels, shape, num_cells_per_validation_tensor, start_idx_val, end_idx_val)
            np.save(os.path.join(val_tensor_path, f'tensor_{i}.npy'), tensorVal)
            np.save(os.path.join(val_tensor_path, f'labels_{i}.npy'), labelsVal)
            del tensorVal 
            del labelsVal 
            ...
            # Test set
            start_idx_test = int(16 * (num_cells_per_training_tensor + num_cells_per_validation_tensor)) // 4 + int(i * num_cells_per_test_tensor) // 4
            end_idx_test = int(16 * (num_cells_per_training_tensor + num_cells_per_validation_tensor)) // 4 + int((i + 1) * num_cells_per_test_tensor) // 4
            #print(f"{Colors.GREEN}start idx test is: {start_idx_test}{Colors.RESET}")
            #print(f"{Colors.GREEN}end idx test is: {end_idx_test}{Colors.RESET}")
            tensorTest, labelsTest = create_tensor(path_to_seg_file, channels, shape, num_cells_per_test_tensor, start_idx_test, end_idx_test)
            np.save(os.path.join(test_tensor_path, f'tensor_{i}.npy'), tensorTest)
            np.save(os.path.join(test_tensor_path, f'labels_{i}.npy'), labelsTest)
            del tensorTest 
            del labelsTest 
            gc.collect() 